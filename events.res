...
beginstep: 3
	...
	Mode: Special
	Case: 1
	Constant: {xprevious = x; yprevious = y; if (sprite_index != -1 && image_single < 0) image_index=fmod((image_speed < 0)?(sprite_get_number(sprite_index) + image_index - fmod(abs(image_speed),sprite_get_number(sprite_index))):(image_index + image_speed), sprite_get_number(sprite_index)); }
...
collision: 4
	...
	prefix: object_basic* lastInst=0; for (enigma::iterator it = enigma::fetch_inst_iter_by_int(%1); it; ++it) {lastInst=*it;} bool last=false; for (enigma::iterator it = enigma::fetch_inst_iter_by_int(%1); it && !last; last=(*it==lastInst), ++it) {int $$$internal$$$ = %1; instance_other = *it; if (enigma::place_meeting_inst(x,y,instance_other->id)) {if(enigma::glaccess(int(other))->solid && enigma::place_meeting_inst(x,y,instance_other->id)) x = xprevious, y = yprevious;
...
draw: 8
	...
	Default: if (visible && sprite_index != -1) draw_sprite_ext(sprite_index,image_index_actual(),x,y,image_xscale,image_yscale,image_angle,image_blend,image_alpha);
...
animationend: 7
	...
	Mode: Special
	Case: 7
	Sub Check: { return !(image_index_actual() + image_speed < sprite_get_number(sprite_index)); }
