...
struct lang_CPP: language_adapter {
  ...
  int compile_parseAndLink(EnigmaStruct*,parsed_script*[], vector<parsed_script*>& tlines);
  int compile_parseSecondary(map<int,parsed_object*>&,parsed_script*[],int scrcount, vector<parsed_script*>& tlines, map<int,parsed_room*>&,parsed_object*, const std::set<std::string>&, const std::map<std::string, std::set<std::string> >& object_local_script_names);
